...
class Wallet4Sales
{
	 ...
	 * @return bool|string
	 */
	public function create($output = false, $Nombre = ''){
		...
		if($output == true) {
			...
			$fileName = $this->getName() ? $this->getName() : $Nombre.$name;
			if(!strstr($fileName, '.')) {
					$fileName .= '.pkpass';
			}
	...
	public function getTempPathCertificates(){
		return $this->TempPathCertificates;
	}

	public function setTempPathCertificates($TempPathCertificates){
		$this->TempPathCertificates = $TempPathCertificates;
	}
	...
	public function GetContenttoPEM($pem){
		$fp = fopen($pem,"r");
		$cert = fread($fp, 8192);
		fclose($fp);
